...
- **URL** (tool `url`): non-empty, ≤256 chars, must be an `http(s)` URL. `{{var}}` placeholders are allowed only in the **path/query** (substituted then percent-encoded at call time); the **authority (scheme/userinfo/host/port) must be literal** — both a fully-templated authority (`{{base}}/x`) and a templated host (`https://{{host}}/x`, `https://user:{{pw}}@h/x`) are rejected, so an untrusted caller arg can't redirect the request (with the tool's configured secret headers) to a host of its choosing.
...
**Injection-safe substitution**: values substituted into the **URL** are percent-encoded (`encodeURIComponent`) and values substituted into **cookies** (the cookies field *and* a `Cookie` header) are cookie-safe-encoded (strip CR/LF, encode `;`/`,` only — base64/JWT chars `+` `/` `=` are preserved) so an untrusted caller arg can't inject extra query params / path segments or smuggle a second cookie. (Config `params` are encoded by `URLSearchParams.set`; other header values are CRLF-rejected by the runtime `Headers`.) Consequence: a URL's scheme+host must be literal — a value used as the whole authority would be percent-encoded. Note: raw-json **body** substitution is not JSON-escaped, so a tool exposing `{{var}}` inside a JSON string still trusts the caller's value there.
...
**Timeout / redirects**: upstream `fetch` is bound by a 30s `AbortSignal.timeout` (slow/hanging upstream aborts → generic error). Redirects are refused: Workers' `fetch` does not support `redirect: 'error'` (it throws `Invalid redirect value…`), so we use `redirect: 'manual'` (returns the raw 3xx without following) and then throw on any WHATWG redirect status (301/302/303/307/308) → generic error. This keeps the internal-host guard (initial URL only) from being bypassed by an external URL redirecting to an internal host. (Following redirects with per-hop host validation is a possible future enhancement.)
